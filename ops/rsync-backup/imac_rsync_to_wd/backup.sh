#!/bin/zsh
set -euo pipefail
chmod +x /Users/jiali/Ops/rsync-backup/bin/backup.sh

# 加载配置
# ===== Load config =====
SCRIPT_DIR="${0:A:h}"            # zsh: 脚本所在目录
PROJECT_DIR="${SCRIPT_DIR:h}"    # 上一级（假设脚本在 bin/ 里）
ENV_FILE="${PROJECT_DIR}/conf/backup.env"
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  REMOTE_TRASH_DIR="${REMOTE_DEST_PATH}/.rsync-trash/${TS}"
