下面是我给你“调出”的 finder_tags_export_csv.py（可直接复制保存成同名文件使用）。它会递归扫描目录，把 Finder 标签导出到 CSV：
读取扩展属性：com.apple.metadata:_kMDItemUserTags
兼容：优先用 os.getxattr，如果你的 Python 环境没有，再自动 fallback 到 xattr -p
CSV 字段：path, relative_path, name, is_dir, size_bytes, mtime_iso, tags, tag_colors
用法示例：
python3 finder_tags_export_csv.py "/Volumes/YourHDD/Movies" "/Users/jerry/Desktop/finder_tags.csv"
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
if __name__ == "__main__":
    main()
快速使用指令（你可以直接复制）
# 例：导出外置盘某目录的 Finder tags 到桌面
python3 finder_tags_export_csv.py "/Volumes/Cold" "$HOME/Desktop/finder_tags_export.csv" --relative --include-dirs